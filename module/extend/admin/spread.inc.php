...
require MOD_ROOT.'/spread.class.php';
$do = new spread();
$menus = array (
...
switch($action) {
	...
	case 'html':		
		if(!isset($fid)) {
			$r = $db->get_one("SELECT min(itemid) AS fid FROM {$DT_PRE}spread");
			$fid = $r['fid'] ? $r['fid'] : 0;
		}
		if(!isset($tid)) {
			$r = $db->get_one("SELECT max(itemid) AS tid FROM {$DT_PRE}spread");
			$tid = $r['tid'] ? $r['tid'] : 0;
		}
		$num = 50;
		if($fid <= $tid) {
			$result = $db->query("SELECT itemid,mid FROM {$DT_PRE}spread WHERE itemid>=$fid ORDER BY itemid LIMIT 0,$num ");
