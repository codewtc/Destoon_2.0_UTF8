...
switch($action) {
	case 'login':
		...
		$dt_password = preg_match('/^\w{32}$/', $password) ? md5($password) : md5(md5($password));
		...
		if( $uid == -1 ) {/* Ucenter 用户不存在或被删除 */
			if($user) {
				if($user['password'] !=  $dt_password) message('密码错误，请重试');
			...
			if($user) {
				if($user['password'] == $dt_password) { /* 更新UC密码 */
			...
			if($user) {
				/* 同步DT密码 */
				if($user['password'] != $dt_password) $db->query("UPDATE {$DT_PRE}member SET password='$dt_password' WHERE username='$username'");
			} else {/* 会员不存在 */
