...
switch($action) {
	...
	default:
		if($submit) {
			...
			if($keyword) $condition .= " AND word LIKE '%$keyword%'";
...
class keyword {
	...
	function add($post) {
		...
		$this->db->query("INSERT INTO {$this->table} (moduleid,word,letter,items,total_search,month_search,week_search,today_search,updatetime,status) VALUES('$post[moduleid]','$post[word]','$post[letter]','$post[items]','$post[total_search]','$post[month_search]','$post[week_search]','$post[today_search]','$DT_TIME', '$post[status]')");
	}
	...
	function edit($post) {
		foreach($post as $k=>$v) {
			...
			$this->db->query("UPDATE {$this->table} SET word='$v[word]',letter='$v[letter]',total_search='$v[total_search]',month_search='$v[month_search]',week_search='$v[week_search]',today_search='$v[today_search]',status='$v[status]' WHERE itemid='$k'");
		}
	}
