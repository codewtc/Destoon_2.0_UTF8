...
<tr id="tr_option" style="display:<?php in_array($html, array('select', 'radio', 'checkbox')) ? '' : 'none';?>">
<td class="tl">选项值</td>
<td>
<textarea name="post[option_value]" style="width:250px;height:80px;overflow:visible;"><?php echo $option_value;?></textarea>
一行一个 选项值|选项名称* 注意*为结尾标志符
</td>
</tr>
<tr id="tr_thumb" style="display:none;">
<td class="tl">默认宽高</td>
<td><input name="post[width]" type="text" id="width" size="3" value="<?php echo $width;?>"/> px X <input name="post[height]" type="text" id="height" size="3" value="<?php echo $height;?>"/> px</td>
</tr>
<tr>
<td class="tl">输入限制</td>
...
function dhtml(id) {
	if(id == 'select' || id == 'radio' || id == 'checkbox') {
		...
		$('tr_thumb').style.display = 'none';		
	} else if(id == 'thumb') {
		$('tr_option').style.display = 'none';
		$('tr_thumb').style.display = '';
	} else {
		$('tr_option').style.display = 'none';
		$('tr_thumb').style.display = 'none';
	}
