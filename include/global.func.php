...
function dsafe($string) {
	if(is_array($string)) {
		...
		$search = array("/&#([a-z0-9]+)([;]*)/i", "/(j[\s\r\n\t]*a[\s\r\n\t]*v[\s\r\n\t]*a[\s\r\n\t]*s[\s\r\n\t]*c[\s\r\n\t]*r[\s\r\n\t]*i[\s\r\n\t]*p[\s\r\n\t]*t|jscript|vbscript|expression|script|frame|link|import)/i", "/on(mouse|exit|error|click|dblclick|key|load|unload|change|move|submit|reset|cut|copy|select|start|stop)/i");
...
function banip($IP) {
	...
	if($ban) message('IP '.$DT_IP.' 已经被网站禁止 如有疑问请联系方式');
}
...
function banword($WORD, $string, $extend = true) {
	...
	foreach($WORD as $v) {
		if($v['deny'] && $extend) {
			if(preg_match("/".$v['replacefrom']."/i", $string)) message('提交的内容含有被网站禁止的字符');
		} else {
...
function tag($parameter, $expires = 0) {
	global $DT, $CFG, $MODULE, $CATEGORY, $db, $DT_PRE, $DCAT, $DT_TIME, $_tags;
	...
	if($areaid) {
		if(is_numeric($areaid)) {
			if($areachild) $AREA = cache_read('area.php');
